...
1. 設計(バイブコーディング): パイプライン定義 → インターフェース定義 → API 詳細定義
2. インターフェース確定後、**API ごとにエージェンティックコーディング**
3. DDL 確定 → マイグレーション作成
4. ステージ単位の実装(Ingestion → Query の順)
